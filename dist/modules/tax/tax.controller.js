...
export class TaxController {
    ...
    static async calculateTax(req, res) {
        try {
            ...
            const taxResult = await TaxService.calculateTransactionTax(userId, portfolioId, transactionType, symbol, quantity, price, new Date(date));
            return ResponseHelper.success(res, taxResult, 'Tax calculated successfully');
        }
        catch (error) {
            const statusCode = error.statusCode || 500;
            return ResponseHelper.error(res, null, error.message || 'Failed to calculate tax', statusCode);
        }
    }
    /**
    ...
    static async getTaxReport(req, res) {
        try {
            ...
            const statusCode = error.statusCode || 500;
            return ResponseHelper.error(res, null, error.message || 'Failed to generate tax report', statusCode);
        }
    }
    /**
     * Get user tax summary (all portfolios)
     */
    static async getUserTaxSummary(req, res) {
        try {
            const userId = req.user.id;
            const startDate = req.query.startDate
                ? new Date(req.query.startDate)
            ...
            const statusCode = error.statusCode || 500;
            return ResponseHelper.error(res, null, error.message || 'Failed to get tax summary', statusCode);
        }
    }
    /**
     * Add tax deduction to a transaction
     */
    static async addTaxDeduction(req, res) {
        try {
            const userId = req.user.id;
            ...
            const deduction = await TaxService.addTaxDeduction(transactionId, category, amount, description);
            return ResponseHelper.success(res, deduction, 'Tax deduction added successfully', 201);
        }
        catch (error) {
            const statusCode = error.statusCode || 500;
            return ResponseHelper.error(res, null, error.message || 'Failed to add tax deduction', statusCode);
        }
    }
    /**
     * Get tax deductions for a transaction
     */
    static async getTransactionTaxDeductions(req, res) {
        try {
            const userId = req.user.id;
            const { transactionId } = req.params;
            ...
            const statusCode = error.statusCode || 500;
            return ResponseHelper.error(res, null, error.message || 'Failed to get tax deductions', statusCode);
        }
    }
}
