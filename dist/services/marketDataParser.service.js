...
export class MarketDataParserService {
        ...
        try {
            ...
            const getStatValue = (label) => {
                ...
                if (parsed === null && valueText) {
                    logger.debug(`Failed to parse value "${valueText}" for label "${label}" for symbol ${symbol}`);
                }
                return parsed;
            };
            ...
            data.askVolume = parseNumber(regPanel
                .find('.stats_item')
                .filter((_, el) => {
                const label = $(el).find('.stats_label').text().trim();
                return (label === 'Ask Volume' || label === 'Ask' || label.toLowerCase() === 'ask volume');
            })
                .find('.stats_value')
                .text()
                .trim());
            data.bidPrice = getStatValue('Bid Price') || getStatValue('Bid') || getStatValue('BID PRICE');
            data.bidVolume = parseNumber(regPanel
                .find('.stats_item')
                .filter((_, el) => {
                const label = $(el).find('.stats_label').text().trim();
                return (label === 'Bid Volume' || label === 'Bid' || label.toLowerCase() === 'bid volume');
            })
                .find('.stats_value')
                .text()
                .trim());
            ...
            if (changeValueDiv.length > 0) {
                ...
                if (data.change !== null) {
                    logger.debug(`Extracted change value "${changeText}" -> ${data.change} for symbol ${symbol}`);
                }
            }
            if (changePercentDiv.length > 0) {
                ...
                if (data.changePercent !== null) {
                    logger.debug(`Extracted change percent "${changePercentText}" -> ${data.changePercent} for symbol ${symbol}`);
                }
            }
     ...
     */
    static async fetchAndParse(symbol, url) {
        try {
...
//# sourceMappingURL=marketDataParser.service.js.map