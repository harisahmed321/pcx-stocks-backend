...
export class TransactionsService {
  static async createTransaction(userId: string, portfolioId: string, data: CreateTransactionDto) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (data.type === 'BUY' || data.type === 'BONUS') {
        ...
        if (existingHolding) {
          ...
          const newAvgPrice =
            (Number(existingHolding.avgBuyPrice) * Number(existingHolding.quantity) + data.price * data.quantity) /
            newQuantity;
  ...
  static async getPortfolioTransactions(userId: string, portfolioId: string, page: number = 1, limit: number = 20) {
    // Verify portfolio ownership
    const portfolio = await prisma.portfolio.findFirst({
      where: { id: portfolioId, userId },
    });
